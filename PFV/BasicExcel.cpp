...
// PURPOSE: Read a block of data the opened file at the index position.
...
// Create a new Excel workbook on a basis of given object
void BasicExcel::New(const BasicExcel &xls)
{
	workbook_ = xls.workbook_;
	worksheets_.clear();

	workbook_.fonts_ = xls.workbook_.fonts_;
	for (auto &it : workbook_.fonts_)
		it.colourIndex_ = 0x7FFF;						// repair font colors to black
	workbook_.XFs_ = xls.workbook_.XFs_;
	for (auto &it : workbook_.XFs_)
	{
		it.borderLines_ = 0x0000;	// no borderlines
		it.colour1_ = 0x0000;		// set default color
		it.colour2_ = 0x20C0;		// set default color
	}
	workbook_.styles_ = xls.workbook_.styles_;
	workbook_.boundSheets_ = xls.workbook_.boundSheets_;
	worksheets_ = xls.worksheets_;
	...
	for (int sheet_cnt = 0; sheet_cnt < GetTotalWorkSheets(); ++sheet_cnt)
	{
		...
		for (int row = 0; row < sheet->maxRows_; ++row)
		{
			...
			{
				sheet->Cell(row, col)->EraseContents();
			}
		}
	}
}
...
BasicExcelCell* BasicExcelWorksheet::Cell(int row, int col)
{
    ...
    BasicExcelCell *cell = &(cells_[row][col]);
...
void BasicExcelWorksheet::AddTextToPreviousCell(BasicExcelCell *cell)
{
    int row = cell->GetRow();
    int col = cell->GetCol();
	std::string prev_cell_string, cell_string;
...
void BasicExcelWorksheet::DeleteEmptyRows(int row, int empty_index)
{
	...
	//std::cout << "row: " << row << " empty index: " << empty_index << std::endl;
...
bool BasicExcelWorksheet::czy_zawiera_uklad(std::string napis, std::vector<std::string> vUklady)
{
    for(std::vector<std::string>::iterator it_uklady = vUklady.begin(); it_uklady != vUklady.end(); ++it_uklady)
    {
        if(napis.find(*it_uklady) == 0)
            return true;
    }
    return false;
}
...
void BasicExcelWorksheet::ParseCells(int row, const std::vector<std::string> vUklady, const int max_len)
{
	std::string temp;
    ...
    if(czy_zawiera_uklad(text, vUklady))
    {
		...
		std::cout << "zawiera układ... sprawdzam nastepny wiersz..." << std::endl;
        ...
        cell->EraseContents();          /// komorka nie zawiera ukladu

    return ;
}
/*
BasicExcelCell *BasicExcelWorksheet::GetCellWithText(const std::string &search_text)
{
    BasicExcelCell* cell;
	...
	//std::cout << "Nie znaleziono tekstu: " << search_text << std::endl;
    return &(cells_[0][0]);
}
*/
BasicExcelCell *BasicExcelWorksheet::GetCellWithInteger(const int &search_int, const RangeOfCells &cell_range)
{
	...
	//std::cout << "Nie znaleziono liczby: " << search_int << " w zakresie [row][col]: [" << cell_range.getR_start() << "-" << cell_range.getR_end() << "][" << cell_range.getC_start() << "-" << cell_range.getC_end() << "]" << std::endl;
    return &(cells_[0][0]);
}
...
/*
BasicExcelCell::BasicExcelCell(const BasicExcelCell & other)
{
	*this = other;
}
...
BasicExcelCell::BasicExcelCell(BasicExcelCell&& other)
{
	*this = std::move(other);
}
...
RangeOfCells::RangeOfCells(const unsigned short &row_start, const unsigned short &row_end, 
	const unsigned short &col_start, const unsigned short &col_end)
{
    if(row_start > row_end || col_start > col_end)
    {
		std::cout << "Nieprawidłowy zakres danych w konstruktorze RangeOfCells" << std::endl;
    }
    else
    {
...
int BasicExcelCell::GetCol() const
{
    return col_;
}

void BasicExcelCell::SetCol(int col)
{
    col_ = col;
}

int BasicExcelCell::GetRow() const
{
    return row_;
}

std::string BasicExcelCell::GetRowCol() const
{
	return std::string("[" + std::to_string(row_+1) + ", " + std::to_string(col_+1) + "]");
}

void BasicExcelCell::SetRow(int row)
{
    row_ = row;
}


/*bool BasicExcelCell::CheckCellForText(string text)
{
    string cell_String, text2 = text;
    wstring cell_WString, wtext;
    bool ret_flag = false;

    //cout << "funkcja CheckCellForText: przeslano -> " << text << endl;
    ...
    {
        text.erase(text.find(" "));                                     //usuwa czesc nazwy i przypisuje do name2; name jest teraz poj. wyrazem
        text2.erase(0, text2.find(" ")+1);
        //cout << "\tRozdzielam! teraz jest t1: " << text << " oraz t2: " << text2 << endl;
    }

    text[0] = toupper(text[0]);          //pierwsza litera na
...
/*
size_t BasicExcelCell::CheckCellForText(std::string text)
{
    ...
    return cell_String.find(text);

}
*/
std::string BasicExcelCell::ConvertCellToString()
{
...
std::wstring BasicExcelCell::GetText() const
{
	if (type_ == INT || type_ == DOUBLE)
		return std::to_wstring(ival_);
	else if (type_ == STRING)
		return ::StringToWString(GetString());
