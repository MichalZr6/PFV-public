...
#include <sstream>
#include <stdio.h>
#include "Functions.h" // MZ 09.04.2022 for wstring <-> string convertions
...
class RangeOfCells
{
    ...
    RangeOfCells();
    RangeOfCells(const unsigned short &row_start, const unsigned short &row_end, const unsigned short &col_start, const unsigned short &col_end);

    unsigned short getR_start() const;
    unsigned short getR_end() const;
    unsigned short getC_start() const;
    unsigned short getC_end() const;

    void Modify(unsigned short row_start = 0, unsigned short row_end = 0, unsigned short col_start = 0, unsigned short col_end = 0);

    private:

    unsigned short _row_start;
    unsigned short _row_end;
    unsigned short _col_start;
    unsigned short _col_end;
};
...
public: // Worksheet functions
    ...
    void InsertRow(int RowNumber, int NumberOfRows, int colRange=15);
...
public: // Cell functions
    ...
    ///MOJE FUNKCJE
    BasicExcelCell *GetCellWithText(const std::string &search_text);
    BasicExcelCell *GetCellWithInteger(const int &search_int, const RangeOfCells &cell_range);
    BasicExcelCell *GetFirstCellNotEmpty();
    void AddTextToPreviousCell(BasicExcelCell *cell);
    void DeleteRows(int row_start, int row_end);
	void DeleteEmptyRows(int row, int empty_index);
    void ParseCells(int row, const std::vector<std::string> vUklady, const int max_len = 90);
    bool czy_zawiera_uklad(std::string napis, std::vector<std::string> vUklady);
...
public:
	BasicExcelCell();
	//BasicExcelCell(const BasicExcelCell &other);
	//BasicExcelCell::BasicExcelCell(BasicExcelCell &&other);
	//BasicExcelCell &operator=(const BasicExcelCell &other);
	...
	std::wstring GetText() const;		///< Get std::wstring (numbers are converted)
	std::string GetDate() const;		///< MZ: gets date dd.mm.yyyy or empty string
    ...
    void SetCol(int);
	int GetRow() const;
	std::string GetRowCol() const;		// uwaga: w przeciwieñstwie do powy¿szych, zwraca adres wzglêdny
    void SetRow(int);
    //size_t CheckCellForText(std::string text);
	std::string ConvertCellToString();
    void SetCell(const BasicExcelCell *cell);		/// set values of current cell to given cell
